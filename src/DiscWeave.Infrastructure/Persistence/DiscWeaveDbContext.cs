...
using DiscWeave.Infrastructure.Persistence.Configurations;
...
public partial class DiscWeaveDbContext : IdentityDbContext<DiscWeaveUser, IdentityRole<Guid>, Guid>, IUnitOfWork
{
    ...
    public DiscWeaveDbContext(DbContextOptions<DiscWeaveDbContext> options)
        : base(options)
    {
    }

    public DiscWeaveDbContext(DbContextOptions<DiscWeaveDbContext> options, ICurrentCollection currentCollection)
        : base(options)
    {
    ...
    {
        _ = builder.Entity<DiscWeaveUser>(user =>
        {
