...
backup_dir="${DISCWEAVE_RESTORE_DRILL_DIR:-$repo_root/artifacts/restore-drills/$timestamp}"
source_project="${DISCWEAVE_RESTORE_DRILL_SOURCE_PROJECT:-discweave_backup_source}"
target_project="${DISCWEAVE_RESTORE_DRILL_TARGET_PROJECT:-discweave_restore_drill_$timestamp}"
...
wait_for_postgres() {
  ...
  for _ in {1..30}; do
    if docker compose --env-file "$env_file" -p "$project" -f "$compose_file" exec -T postgres \
      pg_isready -h 127.0.0.1 -U discweave -d discweave >/dev/null 2>&1; then
      return 0
    fi
...
docker compose --env-file "$env_file" -p "$source_project" -f "$compose_file" exec -T postgres \
  pg_dump -h 127.0.0.1 -U discweave -d discweave -Fc > "$backup_dir/postgres.dump"

echo "Creating service-storage backup artifact"
docker run --rm \
  -v "${source_project}_discweave-service-data:/data:ro" \
  -v "$backup_dir:/backup" \
  alpine:3.20 \
...
docker compose --env-file "$env_file" -p "$target_project" -f "$compose_file" exec -T postgres \
  pg_restore --clean --if-exists --no-owner -h 127.0.0.1 -U discweave -d discweave < "$backup_dir/postgres.dump"

echo "Restoring service-storage backup into drill target"
docker run --rm \
  -v "${target_project}_discweave-service-data:/data" \
  -v "$backup_dir:/backup" \
  alpine:3.20 \
...
docker compose --env-file "$env_file" -p "$target_project" -f "$compose_file" exec -T postgres \
  psql -h 127.0.0.1 -U discweave -d discweave -v ON_ERROR_STOP=1 \
