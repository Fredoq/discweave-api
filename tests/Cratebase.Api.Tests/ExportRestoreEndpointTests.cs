...
public sealed partial class ExportRestoreEndpointTests : IClassFixture<PostgresFixture>
{
    ...
    public async Task Json_export_and_restore_include_naming_profiles_and_release_overrides()
    {
        ...
        Assert.Contains(profileId.ToString(), restoredSnapshot, StringComparison.OrdinalIgnoreCase);
        Assert.Contains(releaseId.ToString(), restoredSnapshot, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("\"source\":\"WEB\"", restoredSnapshot, StringComparison.Ordinal);
    }
