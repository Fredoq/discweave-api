...
public sealed class HealthEndpointTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    public async Task The_health_endpoint_returns_the_service_status()
    {
        ...
        const string connectionStringVariableName = "ConnectionStrings__Cratebase";
        string? previousConnectionString = Environment.GetEnvironmentVariable(connectionStringVariableName);
        Environment.SetEnvironmentVariable(connectionStringVariableName, "Host=localhost;Database=cratebase;Username=cratebase");
