...
public sealed class DesktopImportEndpointTests : IClassFixture<PostgresFixture>
{
    ...
    public async Task Desktop_download_returns_the_configured_macos_installer()
    {
        ...
        string installerPath = Path.Combine(installerDirectory.Path, "DiscWeave-0.0.0-arm64.dmg");
        ...
        Assert.Equal("DiscWeave-0.0.0-arm64.dmg", response.Content.Headers.ContentDisposition?.FileName?.Trim('"'));
