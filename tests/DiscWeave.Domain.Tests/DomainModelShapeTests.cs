...
public sealed class DomainModelShapeTests
{
    ...
    private static bool IsDomainNamespace(string? namespaceName)
    {
        return namespaceName is not null &&
            (namespaceName == "DiscWeave.Domain" ||
                namespaceName.StartsWith("DiscWeave.Domain.", StringComparison.Ordinal)) &&
            namespaceName != "DiscWeave.Domain.Imports" &&
            !namespaceName.StartsWith("DiscWeave.Domain.Imports.", StringComparison.Ordinal);
    }
